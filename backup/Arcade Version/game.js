...
const altSpriteSources = {
  washington: {
    ...
    walk: { src: "assets/sprites/washington-walk.png", nativeSide: 1, scale: 1.17 },
  },
};
...
function drawFighterSprite(f, sprite) {
  ...
  // forward-walk pose: fake a stride cycle (bounce + rock + footfall squash)
  // since it's a single photo, not a real walk-cycle sprite sheet.
  if (poseKey === "walk") {
    const stride = Math.sin(f.bob * 2);
    extraY -= Math.abs(stride) * 9;
    rotate += stride * 0.05 * f.side;
